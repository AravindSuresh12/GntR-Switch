...
function precompute_translation_parameter_array_bacteria(biophysical_dictionary::Dict{String,Any}, protein_coding_length_array::Array{Float64,1},  time_constant_modifier_array::Array{Float64,1})::Array{TranslationParameters,1}
    ...
    species_symbol_type_array = biophysical_dictionary["species_symbol_type_array"] #x here refers to within biophysical dictionary about their type classification, if they are protein/gene/whatever the fuck they are
    ...
    for (index,protein_length) in enumerate(protein_coding_length_array)
        ...
        # compute kI -
        kI = (1/characteristic_initiation_time_translation)  #initiation is 
...
#ARAVIND adding a new function to calculate Tx sum for transcription- Not doing this for Kx bv

#function name- time sum

#input- takes in the gene array

#output, a single float number which is used as the sum of time constants of all genes 

# function time_sum(gene_coding_length_array::Array{Float64,1},  time_constant_modifier_array::Array{Float64,1}, biophysical_dictionary::Dict{String,Any}, gene_abundance_array::Array{Float64,1} )

#     #getting some stuff for array



#     characteristic_length = biophysical_dictionary["characteristic_protein_length"]
...
#     KX_factor = biophysical_dictionary["transcription_saturation_constant"]
...
#         time_scale_parameter = time_constant_modifier_array[correction_index]
#         saturation_constant_parameter=time_constant_modifier_array[correction_index]
...
#          # compute the tau factor -
#         tau_factor = (kE/kI)*time_scale_parameter
#         KX=KX_factor*saturation_constant_parameter

#         time= time+ (tau_factor*KX) #AGAIN, this can technically be vectorized, but I prefer the non vectorized approach
#     end

#     return(time)
# end

#Need to manually look at the units now... for OLJ term


# #translation OLJ calculation function

# #takes in: biophysical_dictionary 

#returns out a term that has t_sum

# function translation_OLJ(biophysical_dictionary::Dict{String,Any}, mRNA_coding_length_array::Array{Float64,1},time_constant_modifier_array::Array{Float64})

#     characteristic_length = biophysical_dictionary["characteristic_transcript_length"]
#     translation_elongation_rate = biophysical_dictionary["translation_elongation_rate"]
#     characteristic_initiation_time = biophysical_dictionary["characteristic_initiation_time_translation"]
#     KL_factor = biophysical_dictionary["translation_saturation_constant"]
#     ribosome_concentration = biophysical_dictionary["ribosome_concentration"]
#     species_symbol_type_array = biophysical_dictionary["species_symbol_type_array"]

#      # which indices are protein (the production of translation)?

#      idx_protein = findall(x->x==:protein,species_symbol_type_array)
#      time=0.0

#      for (mRNA_index, mRNA_length) in enumerate(mRNA_coding_length_array)

#         correction_index = idx_protein[mRNA_index]
#         time_scale_parameter = time_constant_modifier_array[correction_index]

#          # compute LE and LI
#         LE = translation_elongation_rate*(1/mRNA_length)
#         LI = (1/characteristic_initiation_time)

#         tau_factor1 = (LE/LI)*time_scale_parameter
#         KL=KL_factor*time_scale_parameter
#         time= time+ (tau_factor1*KL)
#      end
...
function precompute_transcription_kinetic_limit_array_cell_free(biophysical_dictionary::Dict{String,Any}, gene_coding_length_array::Array{Float64,1}, gene_abundance_array::Array{Float64,1},  time_constant_modifier_array::Array{Float64,1})::Array{Float64,1}
    ...
    #only for now I'm assuming that the saturation constant array is same as time constant array.. need to do further fine tuning on this
    # initialize -
   ...
   #sum_time= time_sum(gene_coding_length_array,time_constant_modifier_array,biophysical_dictionary,gene_abundance_array) 
    ...
    for (gene_index, gene_length) in enumerate(gene_coding_length_array, )
        ...
        #OXJ_Term= ( (1 +tau_factor) *G) *(sum_time)/(tau_factor *KX)
        sat_term = (G/ (KX*tau_factor+ (1+tau_factor)*G ))  # OXJ_Term should be here 
...
function calculate_translation_kinetics_array(t,x,mRNA_index_array,translation_parameters_array, data_dictionary)
    ...
    biophysical_dictionary= data_dictionary["biophysical_constants_dictionary"]

    mRNA_coding_length_array= data_dictionary["mRNA_coding_length_array"]

    time_constant_modifier_array= data_dictionary["time_constant_modifier_array"]


    #time= translation_OLJ(biophysical_dictionary, mRNA_coding_length_array,time_constant_modifier_array)
    ...
    for index = 1:number_of_translation_rates
        ...
        tau_new = parameters_struct.tau_factor
        KL = parameters_struct.KL


        #OLJ_Term= (1 + (tau_new*mRNA_concentration) )*(time)/(tau_new*KL)

        # build the rate -
        value = vmax*(mRNA_concentration/(KL*tau_new + (1+tau_new)*mRNA_concentration)) 
...
function calculate_txtl_kinetics_array(t::Float64,x::Array{Float64,1},data_dictionary::Dict{String,Any})::Array{Float64,1} #x refers to the state vectors with x starting from 1.0 to 9.0 because there are 9 compounds
